              ...
              <ul style={styles.list}>{list.map((rel: any, index: number) => {
                ...
                return (
                  <li 
                    ...
                    style={{ ...styles.listItem, opacity: draggedIndex === index ? 0.5 : (past ? 0.4 : 1), cursor: (!isParent && ['setlists', 'master-setlists', 'events', 'tours'].includes(tab)) ? 'grab' : 'default', borderTop: dragOverIndex === index && draggedIndex !== index ? `2px solid ${theme.accent}` : styles.listItem.borderTop, transition: 'border 0.1s' }} 
                    draggable={(!isParent && ['setlists', 'master-setlists', 'events', 'tours'].includes(tab))} 
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      {(!isParent && ['setlists', 'master-setlists', 'events', 'tours'].includes(tab)) && (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                          ...
                          <button style={{ background: 'transparent', border: 'none', color: index === list.length - 1 ? theme.muted : theme.accent, cursor: index === list.length - 1 ? 'default' : 'pointer', padding: 0, fontSize: 10 }} onClick={() => onMove(index, 'down')} disabled={index === list.length - 1}>▼</button>
                        </div>
                      )}
                      {tab === 'setlists' && !isParent && (
                          <button 
                            style={{ background: 'transparent', border: 'none', cursor: 'pointer', marginRight: 8, fontSize: 16 }} 
                            onClick={(e) => {
                                e.stopPropagation();
                                const nextSongId = list[index + 1]?.id || null;
                                console.log('Toggle link:', { songId: rel.id, nextSongId, isCurrentlyLinked: !!rel.linked_to });
                      ...
                      <button style={{ ...styles.button, background: 'transparent', color: theme.danger, border: `1px solid ${theme.danger}` }} onClick={() => onUnassign(rel.id, rel.type)}>Remove</button>
                    </div>
                  </li>
                );
              })}</ul>
